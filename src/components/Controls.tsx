...
const HandConfig = ({
  ...
  return (
    <div className='row'>
      ...
      <input type='range' max={32} min={2} value={strands} onChange={onStrandsChange} />
      <span>{strands}</span>
      <details open>
        ...
        <nav>
          ...
          to
          <Colors type='color' value={colors[0]} onChange={(evt) => {
            onColorChange(
              hand,
        ...
        <div style={{display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(2rem, 1fr))'}}>
...
const Controls: React.FC<any> = () => {
  ...
  return (
    <div id='controls' className='x1 y1 w3 h12 as-table'>
      ...
      <hr className='w12' />
      <div className='row'>
        <label className='full'>Row Count</label>
        <button style={{gridColumn: 'span 4'}} onClick={() => {
          dispatch({
            type: Actions.doWeave,
            payload: { move: -1 } });
          }} disabled={weavingRow === 0}>Prev</button>
        <button style={{gridColumn: 'span 4'}} onClick={() => {
          dispatch({
            type: Actions.doWeave,
            payload: { move: 1 } });
        ...
        <div className='w12'>
          <ul>
            <li><b>Side:</b> {weavingStrand}</li>
            <li><b>Row:</b> {weavingRow + 1}</li>
          </ul>
        </div>
      </div>
      <hr className='w12' />
